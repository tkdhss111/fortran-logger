...
contains
  ...
  subroutine execute_with_logger ( this, file_macro, line_macro, cmd )
    ...
    if ( cmdstat > 0 ) then ! Command execution failed with error
      call this.write ( file_macro, line_macro, '*** Error: cmdstat=', cmdstat, ', cmdmsg:', cmdmsg )
      stop 1
      ...
      if ( exitstat /= 0 ) then ! Command completed with non-zero exitstat
        call this.write ( file_macro, line_macro, '*** Error: exitstat=', exitstat, ', cmdstat=0' )
        stop 1
      else
        call this.write ( file_macro, line_macro, '*** Info: cmdstat=', cmdstat, ', cmdmsg:', cmdmsg )
      end if
    end if
