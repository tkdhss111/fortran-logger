...
contains
  ...
  subroutine exec ( this, file_macro, line_macro, cmd )
    ...
    if ( cmdstat > 0 ) then !Command execution failed with error
      write ( msg, '(a, i0, a)') &
        '*** Error: cmdstat=', cmdstat, ', cmdmsg: '//trim(cmdmsg)
        '*** ErrorCommand execution failed with error: cmdstat=', cmdstat, ', cmdmsg: '//trim(cmdmsg)
      call logger.write ( file_macro, line_macro, msg )
      stop 1
    else if ( cmdstat < 0 ) then
      write ( msg, '(a, i0, a)') &
        '*** Command execution not supported: cmdstat=', cmdstat, ', cmdmsg: '//trim(cmdmsg)
      stop 1
    else ! cmdstat == 0
      write ( msg, '(a, i0, a)') &
        'Command successfully completed: cmdstat=', cmdstat, ', cmdmsg: '//trim(cmdmsg)
      if ( exitstat /= 0 ) then 
        write ( msg, '(a, i0, a)') &
          '*** Command completed with exitstat=', exitstat
        stop 1
      end if
    end if
