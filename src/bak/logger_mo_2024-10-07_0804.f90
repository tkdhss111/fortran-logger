...
contains
  ...
  subroutine open_file ( file_macro, line_macro, u, file, status, access )
    ...
    if ( iostat /= 0 ) then
      call cf.logger.write ( file_macro, line_macro, '*** Erorr on opening file:', trim(iomsg) )
    end if
  end subroutine
