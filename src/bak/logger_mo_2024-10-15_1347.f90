...
contains
  ...
  ! Workaround for namelist slash reading problem in file paths.
  pure function trim_literals ( str ) result ( trimed )
    character(*), intent(in)  :: str
    character(:), allocatable :: trimed
    integer n
    n = len_trim(str)
    if ( str(1:1) == '"' .and. str(n:n) == '"' ) then
      trimed = str(2 : n - 1)
    end if
  end function
